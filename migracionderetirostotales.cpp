...
int main(int __argc, char *__argv[])
{
	...
	{
		shRet = DEFAULT__;
		snprintf(cTexto, sizeof(cTexto), "[%s][%s] INTENTO DE CONEXION: %i", __FILE__, __FUNCTION__, iNumIntento);
		...
		if (shRet == OK__)
		{
			...
			snprintf(cTexto, sizeof(cTexto), "[%s][%s] CONECTADO EN EL INTENTO NUMERO: %i", __FILE__, __FUNCTION__, iNumIntento);
			fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
			break;
		}
		else
	...
	odbcAforeGlobal.Close();
	odbcServiciosAfore.Close();
	snprintf(cTexto, sizeof(cTexto), "[%s][%s] FIN DEL PROCESO...", __FILE__, __FUNCTION__);
	fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
}
...
short obtenerConexionBaseDato()
{
	...
	if (strnlen(cIp1, sizeof(cIp1)) > 0 || strnlen(cIp2, sizeof(cIp2)) > 0)
	{
		...
		snprintf(cTexto, sizeof(cTexto), "[%s][%s] Abriendo conexion a Postgres, AdmonAfore, IP: %s", __FILE__, __FUNCTION__, cIp1);
		fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
		...
		if (shRet != OK__) // si NO se abrió conexion a: admonafore
		{
			...
			snprintf(cTexto, sizeof(cTexto), "[%s][%s] Abriendo conexion a Postgres, AdmonAfore, IP: %s", __FILE__, __FUNCTION__, cIp2);
			fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
			shRet = fns.fnConAbrirConexionPostgres(&odbcAdmonAfo, cIpAdmonAfore, (char *)BDADMONAFORE, (char *)USERADMONAFORE, cTexto);
		}
		...
		{
			snprintf(cTexto, sizeof(cTexto), "[%s][%s] Conexion a Postgres AdmonAfore OK, IP: %s", __FILE__, __FUNCTION__, cIpAdmonAfore);
			fns.fnArcGrabarLogx(RUTA_LOG, cTexto);

			// CONEXION A SERVER AFOREGLOBAL
			memset(cTexto, 0, sizeof(cTexto));
			snprintf(cTexto, sizeof(cTexto), "SELECT TRIM(valor) FROM aforecontrol WHERE Tipo = 1 AND activo = 1 AND id = %i", SERV_DATABASE_AFO_GLOBAL);
			...
			if (shRet == OK__)
			{
				...
				snprintf(cTexto, sizeof(cTexto), "[%s][%s] Abriendo conexion a Postgres, AforeGlobal, IP: %s", __FILE__, __FUNCTION__, cIpAforeGlobal);
				fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
				...
				{
					snprintf(cTexto, sizeof(cTexto), "[%s][%s] Conexion a Postgres AforeGlobal OK", __FILE__, __FUNCTION__);
					fns.fnArcGrabarLogx(RUTA_LOG, cTexto);

					// CONEXION A SERVER AFOREGLOBAL
					memset(cTexto, 0, sizeof(cTexto));
					memset(cOutTexto, 0, sizeof(cOutTexto));
					snprintf(cTexto, sizeof(cTexto), "SELECT concepto2 FROM pafhardcode WHERE tipo1 = 197;");
					shRet = fns.fnSqlConsultarTexto(&odbcAforeGlobal, cTexto, cOutTexto);

					memset(cIpAforeGlobal, 0, sizeof(cIpAforeGlobal));
					memmove(cIpAforeGlobal, cOutTexto, sizeof(cOutTexto));
					snprintf(cTexto, sizeof(cTexto), "[%s][%s] Abriendo conexion a Postgres, serviciosafore, IP: %s", __FILE__, __FUNCTION__, cIpAforeGlobal);
					fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
					...
					if (shRet == OK__)
					{
						snprintf(cTexto, sizeof(cTexto), "[%s][%s] Conexion a Postgres ServiciosAfore OK", __FILE__, __FUNCTION__);
						fns.fnArcGrabarLogx(RUTA_LOG, cTexto);

						// CONEXION A SERVER: INFORMIX
						memset(cTexto, 0, sizeof(cTexto));
						memset(cOutTexto, 0, sizeof(cOutTexto));
						snprintf(cTexto, sizeof(cTexto), "SELECT TRIM(valor) FROM aforecontrol WHERE Tipo = 1 AND activo = 1 AND id = %i", SERV_DATABASE_INFORMIX);
						shRet = fns.fnSqlConsultarTexto(&odbcAdmonAfo, cTexto, cOutTexto);
						if (shRet == OK__)
						{
							...
							snprintf(cTexto, sizeof(cTexto), "[%s][%s] Abriendo conexion a Informix, IP: %s", __FILE__, __FUNCTION__, cIpIfx);
							fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
							...
							if (shRet == OK__)
							{
								snprintf(cTexto, sizeof(cTexto), "[%s][%s] Conexion a Informix OK, IP: %s", __FILE__, __FUNCTION__, cIpIfx);
								fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
							}
							else
							{
		...
		{
			snprintf(cTexto, sizeof(cTexto), "[%s][%s] Excepcion al abrir conexion a Pgs AdmonAfore, IP: %s", __FILE__, __FUNCTION__, cIpAdmonAfore);
			fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
			shRet = ERR_CNX_BASE_DATO;
		}
	}
	...
	{
		snprintf(cTexto, sizeof(cTexto), "%s", "fn: main, EL VALOR DE LA IP DE POSTGRESS NO ES VALIDO O ESTA VACIO, VERIFIQUE...");
		fns.fnArcGrabarLogx(RUTA_LOG, cTexto);
		shRet = DEFAULT__;
	}
