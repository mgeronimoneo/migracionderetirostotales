...
short CInfoRetNotifVivienda::ReplicarRetNotifVivienda(int iFolioServicio, C_ODBC *odbcInfxSafreaf, C_ODBC *odbcServiciosAfore)
{
	...
	snprintf(cSql, sizeof(cSql), "SELECT  TRIM(COALESCE(cnss,''))::char(11) AS cnss, "
								 ...
								 "FROM fnconsultaretnotificaviviendareplicart(%i);",
			 iFolioServicio);
	...
	if (objRetNotViv.Exec(cSql))
	{
		...
		if (objRetNotViv.leer())
		{
			...
			memmove(cNss, objRetNotViv.cNss, sizeof(cNss));
			// memcpy(cFolionotifica, objRetNotViv.cFolionotifica, 29);
			snprintf(cFoliooperacion, sizeof(objRetNotViv.cFoliooperacion), "%s", objRetNotViv.cFoliooperacion);
			snprintf(cClabe, sizeof(objRetNotViv.cClabe), "%s", objRetNotViv.cClabe);
			memmove(cGrupotrabajador, objRetNotViv.cGrupotrabajador, sizeof(cGrupotrabajador));
			memmove(cSecpension, objRetNotViv.cSecpension, sizeof(cSecpension));
			memmove(cRegimen, objRetNotViv.cRegimen, sizeof(cRegimen));
			memmove(cTiporetiro, objRetNotViv.cTiporetiro, sizeof(cTiporetiro));
			memmove(cTiposeguro, objRetNotViv.cTiposeguro, sizeof(cTiposeguro));
			memmove(cTipopension, objRetNotViv.cTipopension, sizeof(cTipopension));
			memmove(cTipoprestacion, objRetNotViv.cTipoprestacion, sizeof(cTipoprestacion));
			memmove(cNombre, objRetNotViv.cNombre, sizeof(cNombre));
			memmove(cPaterno, objRetNotViv.cPaterno, sizeof(cPaterno));
			memmove(cMaterno, objRetNotViv.cMaterno, sizeof(cMaterno));
			memmove(cRfc, objRetNotViv.cRfc, sizeof(cRfc));
			memmove(cCurp, objRetNotViv.cCurp, sizeof(cCurp));
			memmove(cNombrebenef, objRetNotViv.cNombrebenef, sizeof(cNombrebenef));
			memmove(cPaternobenef, objRetNotViv.cPaternobenef, sizeof(cPaternobenef));
			memmove(cMaternobenef, objRetNotViv.cMaternobenef, sizeof(cMaternobenef));
			memmove(cRfcbenef, objRetNotViv.cRfcbenef, sizeof(cRfcbenef));
			memmove(cCurpbenef, objRetNotViv.cCurpbenef, sizeof(cCurpbenef));
