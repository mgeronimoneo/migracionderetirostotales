...
short CInfoRetFolioInfonavit::ReplicarRetFolioInfonavit(int iFolioServicio, C_ODBC *odbcInfxSafreaf, C_ODBC *odbcServiciosAfore)
{
	...
	snprintf(cSql, sizeof(cSql), " SELECT TRIM(COALESCE(cnss,''))::char(11) AS cnss, "
								 ...
								 "FROM fnconsultaretfolioinfonavitreplicart(%i);",
			 iFolioServicio);
	...
	if (objRetFolioInfonavit.Exec(cSql))
	{
		...
		if (objRetFolioInfonavit.leer())
		{
			...
			memmove(cNss, objRetFolioInfonavit.cNss, sizeof(cNss));
			memmove(cFolioInfonavit, objRetFolioInfonavit.cFolioinfonavit, sizeof(cFolioInfonavit));
			memmove(cTipoRetiro, objRetFolioInfonavit.cTiporetiro, sizeof(cTipoRetiro));
			...
			memmove(cUsuarioCaptura, objRetFolioInfonavit.cUsuariocaptura, sizeof(cUsuarioCaptura));
